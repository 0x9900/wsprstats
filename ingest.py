...
def main():
  ...
  if opts.filename:
    ...
    with Timer('Download archive'):
      filename = download_archive(config.wspr_path, month=opts.month, force=opts.force)
  ...
  if not os.path.exists(config.db_name):
    ...
    with DBConnect(config.db_name) as conn:
      curs = conn.cursor()
  ...
  with Timer('Import WSPR'):
    wspr_import(config.db_name, filename, start_id=last_id)
